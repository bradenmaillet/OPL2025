...
and interpret_read (id:string) (mem:memory)
  ...
  (* your code should replace the following line *)
  match inp with
  | [] -> (false, mem, inp, outp @ ["Error: No input available for read"])
  | value :: rest ->
    ( try
        let int_value = int_of_string value in
        let mem2 = List.remove_assoc id mem in
        (true, (id, int_value) :: mem2, rest, outp)
      with Failure _ ->
        (false, mem, inp, outp @ ["Error: Non-numeric input for read"])
    )
...
and interpret_expr (expr:ast_e) (mem:memory) : value * memory =
  ...
  | AST_id id
    ...
    -> (try (Value (List.assoc id mem), mem)
          with Not_found -> (Error ("Undefined variable: " ^ id), mem))
